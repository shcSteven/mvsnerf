...
#     render_poses = [pose_spherical_nerf(rotvec)]
    render_poses = [pose_spherical_nerf(rotvec+np.array([angle,0.0,-phi_range]), 4.0) for angle in np.linspace(-theta_range,theta_range,N_views//4, endpoint=False)]
    render_poses += [pose_spherical_nerf(rotvec+np.array([theta_range,0.0,angle]), 4.0) for angle in np.linspace(-phi_range,phi_range,N_views//4, endpoint=False)]
    render_poses += [pose_spherical_nerf(rotvec+np.array([angle,0.0,phi_range]), 4.0) for angle in np.linspace(theta_range,-theta_range,N_views//4, endpoint=False)]
    render_poses += [pose_spherical_nerf(rotvec+np.array([-theta_range,0.0,angle]), 4.0) for angle in np.linspace(phi_range,-phi_range,N_views//4, endpoint=False)]
...
for i_scene, scene in enumerate(['leaves']):#'horns','flower','orchids', 'room','leaves','fern','trex','fortress'
    ...
    if is_finetued:
        cmd += f'--ckpt ./runs_fine_tuning/{scene}_1h/ckpts//latest.tar --use_disp '
    else:
