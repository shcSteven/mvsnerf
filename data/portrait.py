...
class MVSDatasetPortrait(Dataset):
    ...
    def build_metas(self):
        list = sorted(os.listdir(f'{self.root_dir}/pairs/'))
        if self.split=='train':
            self.scans = [l[:-4] for l in list[0:60]]
        elif self.split=='val':
        ...
        for scan in self.scans:
            #filetemp = codecs.open(f'{self.root_dir}/render_sample75_512/pairs/{scan}.txt', encoding='utf-8')
            #pairs = np.loadtxt(filetemp).astype('int')
            try:
                pairs = np.loadtxt(f'{self.root_dir}/pairs/{scan}.txt').astype('int')
            except UnicodeDecodeError:
                print(f'{self.root_dir}/pairs/{scan}.txt')
            # viewpoints (49)
            for pair in pairs:
                ref_view = int(pair[0])
                src_views = pair[1:]
    ...
    def build_proj_mats(self):
        ...
        for meta in self.metas:
            ...
            proj_mat_filename = os.path.join(self.root_dir,f'cams/{scan}/{ref_view:03d}_cam.txt')
            intrinsic, extrinsic = self.read_cam_file(proj_mat_filename)
            near_far = np.loadtxt(f'{self.root_dir}/minmaxs/{scan}/minmax_map_{ref_view:03d}.txt').tolist()
            # intrinsic[:2] *= 4
    ...
    def __getitem__(self, idx):
        ...
        for i, vid in enumerate(view_ids):
            ...
            img_filename = os.path.join(self.root_dir,
                                        f'images/{scan}/{vid:03d}_courtyard.png')
            depth_filename = os.path.join(self.root_dir,
                                          f'depths/{scan}/{vid:03d}.png')
        ...
        #H = sample['images'].shape[2]
        #W = sample['images'].shape[3]
        #sample['images'] = sample['images'].view(V, H, W, 3)
        #print("images: ", sample['images'].shape)
        sample['depths_h'] = depths_h.astype(np.float32)  # (V, H, W)
        #print("depths_h: ", sample['depths_h'].shape)
        sample['w2cs'] = w2cs.astype(np.float32)  # (V, 4, 4)
        #print("w2cs: ", sample['w2cs'].shape)
